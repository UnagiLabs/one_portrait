...
- **重い実装箇所はモックデータで回避することを検討する。** 2,000枚の投稿収集、モザイク合成、Walrus への大量 PUT、zkLogin / Sponsored Tx の本番フローなど、デモ当日までに実装しきれないと判断した部分は躊躇せずモック / ダミーデータ / 事前生成結果に差し替える。実装に入る前に「本物で通すか、モックで通すか」を必ず判断し、本物にこだわって全体が止まる事態を避ける。
...
ONE Portrait は **Sui + Walrus** 上の非営利オンチェーン共同制作プロジェクト。2,000人のファンが zkLogin + Enoki Sponsored Transaction で写真を1枚ずつ投稿し、2,000枚目の着弾で選手のモザイク肖像画が同期リビールされる。参加者には Soulbound の「欠片 (Kakera)」NFT が配布される。モザイク合成はオフチェーン（Cloudflare Worker + sharp/libvips を乗せた on-demand コンテナ）で行い、完成画像は Walrus に永続化する。投稿履歴・タイル配置・Kakera mint の正本は Move パッケージ `one_portrait`。
...
- contract / registry / AdminCap ID などの非 secret deploy 値は `ops/deployments/testnet.json` を正本にする。
- `ADMIN_SUI_PRIVATE_KEY`、`OP_FINALIZE_DISPATCH_SECRET`、`ENOKI_PRIVATE_API_KEY` は git 管理禁止。local secret は `ops/deployments/testnet.secrets.local.env` に集約する。
- Cloudflare 側では Secrets Store / secret env に local secret と同じ値を設定し、repo には実 secret を置かない。
- `apps/web/.env.local` は個人開発用 override の置き場に限定し、testnet deploy 値や運用 secret の正本にしない。移行期間中は既存ファイルを残してよいが、公開 deploy 値は `ops/deployments/testnet.json`、local secret は `ops/deployments/testnet.secrets.local.env` を正本にする。
