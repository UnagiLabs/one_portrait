...
describe("POST /api/admin/rotate-unit", () => {
  ...
  it("returns 400 for an invalid payload", async () => {
    const response = await POST(
      new Request("http://localhost/api/admin/rotate-unit", {
        body: JSON.stringify({ athleteId: "bad" }),
        headers: {
  ...
  it("relays the validated rotate-unit request to the generator", async () => {
    ...
    expect(request.url).toBe("https://generator.example.com/admin/rotate-unit");
