...
これは full-size unit 向けの fallback 手順です。  
...
前提として、対象 unit は Admin UI の create-unit から作成済みである必要があります。  
display 情報と target image も、その create-unit の中で一緒に設定します。
...
- 対象 unit が Admin UI から作成済みである必要があります。
