...
デモ本番は **demo unit** を使う前提で書きます。  
admin で `demo unit として作成` を選ぶと、`max_slots = 実際に集める残り枚数`、`display_max_slots = 2000` の unit を 1 本作れます。  
ユーザーには `1,995 / 2,000` のような進捗に見え、実送信は `max_slots` 枚 (例: 5 枚) で Filled します。  
`max_slots == display_max_slots` の full-size unit を使う場合は、別途 `docs/demo-seeding.md` で 1,999 件事前投入してから最後の 1 件を残します。
...
| `NEXT_PUBLIC_REGISTRY_OBJECT_ID` | home から active unit を引く registry。`sui_getObject` の `content.fields` に `unit_ids` がある object を使う |
...
| `OP_DEMO_FINALIZE_MANIFEST` | demo unit を finalize するときだけ必須。generator が `display_max_slots - max_slots` 枚の mock tile を読む manifest JSON のパス |
...
demo unit を finalize する場合は、generator 側に `OP_DEMO_FINALIZE_MANIFEST` の manifest を 1 本用意します。  
manifest は `{ "entries": [{ "imageKey": "...", "filePath": "..." }, ...] }` の JSON で、`filePath` は manifest からの相対でも絶対でも読めます。  
`display_max_slots - max_slots` 枚以上の mock 画像を登録しておきます (例: max_slots=5 / display_max_slots=2000 なら 1,995 枚以上)。
...
返ってくる `content.fields` に `unit_ids` がない場合、その object は現行 contract の Registry ではありません。

## 実行手順

1. `corepack pnpm install` を実行します。
2. admin に入り、demo unit を作ります。`demo unit として作成` にチェックし、`実際に集める残り枚数` を小さく (例: 5)、`画面に見せる総数` を 2,000 にします。
3. `corepack pnpm run dev:smoke` を起動します。
4. home から active な athlete card を開きます。画面の進捗が `1,995 / 2,000` のように表示されていることを確認します。
5. ブラウザの DevTools を開き、Network を残します。
6. waiting room で Google login を行います。
7. 投稿に使う写真を選び、同意を入れて送信します。
8. 成功したら画面の `digest` と `送信アドレス` を控えます。
9. Network の Walrus `PUT /v1/blobs?epochs=5` 応答から `blobId` を控えます。
10. waiting room の成功カードで `Kakera を受け取りました。` が出るか確認します。
11. 追加の証跡が必要なら `/gallery` を開き、反映済み entry を確認します。

## preview finalize smoke の実行手順

1. demo unit で試す場合は、generator 側に `OP_DEMO_FINALIZE_MANIFEST` を export し、manifest が `display_max_slots - max_slots` 枚以上の mock 画像を指していることを確認します。
2. `corepack pnpm --filter web run generator:tunnel` を起動します。
3. `generator:tunnel` が local / external `/health` を自動確認して ready になるまで待ちます。
4. `/admin` で current URL と source を確認します。
5. Quick Tunnel を使うときは `.env.local` を直しません。preview Worker は runtime state を自動で読みます。
6. fixed URL を強制したいときだけ shell で `OP_GENERATOR_RUNTIME_URL_OVERRIDE=https://...` を付けます。
7. `corepack pnpm --filter web preview` を起動します。
8. `Filled` の unit id で preview Worker の `/api/finalize` に POST します。demo unit の場合は `max_slots` 枚の実投稿で Filled にしたものを使います。
9. preview Worker の応答を控えます。
10. generator 側で `/dispatch` 到達ログを確認します。demo unit のときは mock tile 読み込みのログが `display_max_slots - max_slots` 枚分出ます。
11. 成功したら `digest` と完成モザイクの `blob_id` を控えます。on-chain `placements` には実投稿分だけが乗ります。
...
demo unit の finalize が `OP_DEMO_FINALIZE_MANIFEST` missing や manifest only has N image(s) の error で止まる場合は、generator 側の env と manifest の画像数を見直します。  
preview finalize で URL が違う場合は `/admin` の current URL、`apps/web/.cache/generator-runtime.json`、shell override の有無を見直します。
...
- demo unit を使う場合は `max_slots < display_max_slots` の unit が admin 経由で作成済み
- demo unit を finalize する場合は generator に `OP_DEMO_FINALIZE_MANIFEST` が設定済みで、manifest に `display_max_slots - max_slots` 枚以上の mock 画像がある
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 2026-04-22 | 未実行 | - | - | - | このローカル環境では `NEXT_PUBLIC_*`、`ENOKI_PRIVATE_API_KEY`、`ADMIN_CAP_ID`、`ADMIN_SUI_PRIVATE_KEY`、`OP_FINALIZE_DISPATCH_SECRET` が未投入で、preview finalize smoke の前提を満たせなかった |
