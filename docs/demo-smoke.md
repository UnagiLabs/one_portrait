...
3. `/admin` で current URL と source を確認します。
4. Quick Tunnel を使うときは `.env.local` を直しません。preview Worker は runtime state を自動で読みます。
5. fixed URL を強制したいときだけ shell で `OP_GENERATOR_RUNTIME_URL_OVERRIDE=https://...` を付けます。
6. `corepack pnpm --filter web preview` を起動します。
