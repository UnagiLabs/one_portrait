...
#[test]
fun finalize_with_primitive_placements_creates_master_updates_unit_and_emits_mosaic_ready_event() {
    let publisher = @0xA11CE;
    let first_submitter = @0xF61;
    let second_submitter = @0xF62;
    let mosaic_blob_id = b"primitive-mosaic-blob";
    ...
    let unit_id = admin_api::create_unit(
        &admin_cap,
        &mut registry,
        b"Demo Athlete Primitive",
        b"https://example.com/primitive.png",
        b"target-blob",
        2,
        2,
        scenario.ctx(),
    ...
    accessors::submit_photo(&mut unit, b"photo-primitive-1", &clock, scenario.ctx());
    ...
    let mut unit = scenario.take_shared_by_id<Unit>(unit_id);
    let clock = scenario.take_shared<Clock>();
    accessors::submit_photo(&mut unit, b"photo-primitive-2", &clock, scenario.ctx());
    test_scenario::return_shared(clock);
    test_scenario::return_shared(unit);
    ...
    admin_api::finalize_with_primitive_placements(
        &admin_cap,
        &mut unit,
        mosaic_blob_id,
        vector[b"photo-primitive-1", b"photo-primitive-2"],
        vector[11, 31],
        vector[21, 41],
        vector[first_submitter, second_submitter],
        vector[1, 2],
        scenario.ctx(),
    );
    ...
    let first_placement = master_portrait::placement_for_testing(&master, b"photo-primitive-1");
    assert_eq!(master_portrait::placement_x_for_testing(&first_placement), 11);
    assert_eq!(master_portrait::placement_y_for_testing(&first_placement), 21);
    assert_eq!(
    ...
    let second_placement = master_portrait::placement_for_testing(&master, b"photo-primitive-2");
    assert_eq!(master_portrait::placement_x_for_testing(&second_placement), 31);
    assert_eq!(master_portrait::placement_y_for_testing(&second_placement), 41);
    assert_eq!(
...
#[test, expected_failure(abort_code = admin_api::EPLACEMENT_VECTOR_LENGTH_MISMATCH)]
fun finalize_with_primitive_placements_rejects_mismatched_lengths() {
    let publisher = @0xA11CE;
    ...
    let unit_id = admin_api::create_unit(
        &admin_cap,
        &mut registry,
        b"Demo Athlete Primitive Mismatch",
        b"https://example.com/primitive-mismatch.png",
        b"target-blob",
        0,
        2,
        scenario.ctx(),
    ...
    admin_api::finalize_with_primitive_placements(
        &admin_cap,
        &mut unit,
        b"primitive-mosaic-blob",
        vector[b"photo-primitive-1"],
        vector[11, 31],
        vector[21],
        vector[publisher],
        vector[1],
